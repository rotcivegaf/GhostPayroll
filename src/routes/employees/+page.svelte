<script lang="ts">
  ...
  let name = '', position = '', salary = '', intmax = '';
...
<div class="container mx-auto p-6">
  ...
  <div class="card bg-base-100 shadow-md p-6 mb-6">
    <h2 class="text-xl font-semibold mb-4">{editId ? 'Edit' : 'Add'} Employee</h2>
    ...
    <div class="form-control mb-2">
      <input class="input input-bordered" bind:value={intmax} placeholder="Intmax Address (optional)" />
    </div>
    <div class="flex gap-2">
      ...
      {#if editId}
        <button class="btn btn-outline" on:click={() => { editId = null; name = position = salary = intmax = '' }}>
          Cancel
        </button>
      {/if}
    </div>
        ...
        {#each employees as emp}
          <tr>
            ...
            <td class="flex gap-2">
              ...
              <button class="btn btn-xs btn-error" on:click={() => deleteEmployee(emp.id)}>Delete</button>
            </td>
          </tr>
        {/each}
      </tbody>
